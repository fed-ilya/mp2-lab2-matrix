...
TEST(TVector, compare_equal_vectors_return_true)
{
	...
	TVector<int> v2(4);
	for (int i = 0; i < v2.GetSize(); i++) v2[i] = 1;
	EXPECT_EQ(v1, v2);
}
...
TEST(TVector, can_add_scalar_to_vector)
{
	...
	for (int i = 0; i < v1.GetSize(); i++) v1[i] = 1;
	TVector<int> v2(4);
	for (int i = 0; i < v2.GetSize(); i++) v2[i] = 4;
	EXPECT_EQ(v1 + 3, v2);
}
...
{
	TVector<int> v1(4);
	for (int i = 0; i < v1.GetSize(); i++) v1[i] = 4;
	TVector<int> v2(4);
	for (int i = 0; i < v2.GetSize(); i++) v2[i] = 1;
	EXPECT_EQ(v1 - 3, v2);
}
...
TEST(TVector, can_multiply_scalar_by_vector)
{
	...
	for (int i = 0; i < v1.GetSize(); i++) v1[i] = 1;
	TVector<int> v2(4);
	for (int i = 0; i < v2.GetSize(); i++) v2[i] = 3;
	EXPECT_EQ(v1 * 3, v2);
}
...
TEST(TVector, can_add_vectors_with_equal_size)
{
	...
	for (int i = 0; i < v2.GetSize(); i++) v2[i] = 3;
	TVector<int> res(4);
	for (int i = 0; i < res.GetSize(); i++) res[i] = 4;
	EXPECT_EQ(v1 + v2, res);
}
...
{
	TVector<int> v1(4);
	for (int i = 0; i < v1.GetSize(); i++) v1[i] = 2;
	TVector<int> v2(4);
	for (int i = 0; i < v2.GetSize(); i++) v2[i] = 3;
	EXPECT_EQ(v1 * v2, 24);
}
